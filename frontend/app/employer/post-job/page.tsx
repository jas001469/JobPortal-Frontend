...
import { useState, useEffect, useRef } from "react";
import { useRouter } from "next/navigation";
import { ExternalLink, Link as LinkIcon, Globe, Briefcase, ChevronRight, FileText, List, Tag, Calendar, MapPin, Search, Edit2, Crown, AlertCircle, CheckCircle, XCircle, ToggleLeft, ToggleRight, Clock, Star } from "lucide-react";
...
// Plan limits based on the actual plan data
const PLAN_LIMITS = {
  "Recruit Basic": {
    jobPostings: 30,
    featuredJobs: 3,
...
export default function PostJobPage() {
  ...
  // Fetch jobs count for total jobs posted
  const fetchJobsCount = async () => {
    try {
      ...
      if (data.success) {
        setJobs(data.jobs);
        setTotalJobsPosted(data.jobs.length);
      }
    } catch (error) {
        ...
        if (SUBSCRIPTION_CONFIG.ENABLED) {
          ...
          if (subscriptionResponse.ok) {
            ...
            if (subData.success && subData.subscription) {
              ...
              // Set the plan limits based on the actual plan
             if (planName && typeof planName === 'string' && PLAN_LIMITS[planName as keyof typeof PLAN_LIMITS]) {
  const limits = PLAN_LIMITS[planName as keyof typeof PLAN_LIMITS];
  setPlanLimits(limits);
  setJobsPosted(subData.subscription.jobsPosted || 0);
  console.log(`Plan: ${planName}`, limits);
} else {
  ...
  // Check if employer can post more jobs based on their plan
  const canPostJob = () => {
    if (!SUBSCRIPTION_CONFIG.ENABLED) return true;
    
    if (!subscription || !planLimits) return false;

    // Compare jobs posted with plan limit
    return jobsPosted < planLimits.jobPostings;
  };

  // Get remaining jobs based on plan
  const getRemainingJobs = () => {
    if (!SUBSCRIPTION_CONFIG.ENABLED) return Infinity;
    
    if (!subscription || !planLimits) return 0;

    return Math.max(0, planLimits.jobPostings - jobsPosted);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canPostJob()) {
      setError(`You have reached your job posting limit of ${planLimits.jobPostings}. Please upgrade your plan to post more jobs.`);
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setJobsPosted(prev => prev + 1);
        setTotalJobsPosted(prev => prev + 1);
  ...
  const planLimit = planLimits?.jobPostings || 0;
    ...
    <div className="min-h-screen bg-gray-50 pt-24 pb-12">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 border border-gray-100">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-1">{totalJobsPosted}</p>
                <p className="text-xs text-gray-400 mt-1">All time postings</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 border border-gray-100">
            <div className="flex items-center">
              <div className="p-3 rounded-full bg-gradient-to-br from-yellow-100 to-yellow-50 mr-4">
                <Clock className="w-7 h-7 text-yellow-700" />
              </div>
              <div>
                <p className="text-sm text-gray-500 font-medium">Draft Jobs</p>
                <p className="text-2xl font-bold text-gray-900 mt-1">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 border border-gray-100">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm text-gray-500 font-medium">Closed Jobs</p>
                <p className="text-2xl font-bold text-gray-900 mt-1">
                  {jobs.filter(j => j.status === "Closed").length || 0}
                </p>
                <p className="text-xs text-gray-400 mt-1">Filled/Expired</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {SUBSCRIPTION_CONFIG.ENABLED && subscription?.status === "active" && planLimits && (
  <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">

    {/* PLAN INFO */}
    <div className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 border border-gray-100 md:col-span-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <div className="p-3 rounded-full bg-gradient-to-br from-red-100 to-red-50 mr-4">
            <Crown className="w-7 h-7 text-red-700" />
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              {remainingJobs} job postings remaining • {planLimits.featuredJobs} featured jobs available
            </p>
          </div>
        </div>
      ...
      <div className="mt-4">
        <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${(jobsPosted / planLimit) * 100}%` }}
          ></div>
        </div>
      </div>
    </div>
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow duration-300 border border-gray-100">
      <div className="flex items-center">
        ...
        <div>
          <p className="text-sm text-gray-500 font-medium">Jobs Used</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">
            {jobsPosted}/{planLimit}
          </p>
          <p className="text-xs text-gray-400 mt-1">
            Job postings used
          </p>
        ...
        {SUBSCRIPTION_CONFIG.ENABLED && !canPost && (
          <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
            <div className="flex items-center">
              <XCircle className="h-5 w-5 text-yellow-600 mr-3 flex-shrink-0" />
              <p className="text-yellow-800">
                You have reached your job posting limit of {planLimit}. Please upgrade your plan to post more jobs.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8">
          ...
          {/* Plan Perks Info - Show current plan benefits */}
          {SUBSCRIPTION_CONFIG.ENABLED && planLimits && (
            <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
              <h3 className="font-semibold text-blue-900 mb-2 flex items-center">
                <CheckCircle className="h-4 w-4 mr-2" />
                Your {planName} Plan Benefits
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div className="flex items-center text-blue-800">
                  <Briefcase className="h-4 w-4 mr-2" />
                  {planLimits.jobPostings} Job Postings ({jobsPosted} used, {remainingJobs} left)
                </div>
                <div className="flex items-center text-blue-800">
                  <Star className="h-4 w-4 mr-2" />
                  {planLimits.featuredJobs} Featured Jobs
                </div>
                <div className="flex items-center text-blue-800">
                  <Calendar className="h-4 w-4 mr-2" />
                  Jobs displayed for {planLimits.jobDisplayDays} days
                </div>
              </div>
            </div>
          )}
          ...
          {canPost ? (
            ...
            <div className="text-center py-12">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Job Posting Limit Reached</h2>
              <p className="text-gray-600 mb-8 max-w-lg mx-auto">
                You have reached the maximum number of job postings ({planLimit}) allowed in your {planName} plan. 
                Please upgrade your subscription to post more jobs.
              </p>
              <button
                onClick={() => router.push("/subscription")}
