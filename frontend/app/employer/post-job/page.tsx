  ...
  useEffect(() => {
    const checkUserAndSubscription = async () => {
      try {
        ...
        // Only check subscription if enabled in config
        if (SUBSCRIPTION_CONFIG.ENABLED) {
          // First check localStorage for active subscription (from cart)
          const localSubscription = localStorage.getItem("activeSubscription");
          
          if (localSubscription) {
            // Use subscription from localStorage
            const parsedSubscription = JSON.parse(localSubscription);
            setSubscription({
              status: "active",
              plan: {
                name: parsedSubscription.plan
              }
            });
            
            // Set job stats from localStorage or initialize
            const savedJobStats = localStorage.getItem("jobStats");
            if (savedJobStats) {
              setJobStats(JSON.parse(savedJobStats));
            }
          } else {
            // If no local subscription, try to fetch from backend
            const subscriptionResponse = await fetch(
              `${process.env.NEXT_PUBLIC_API_URL}/employer/subscription`,
              {
                credentials: "include",
  ...
  const fetchJobStats = async () => {
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem("jobStats", JSON.stringify(data));
      }
    } catch (error) {
      console.error("Error fetching job stats:", error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const updatedStats = {
          ...jobStats,
          totalPosted: jobStats.totalPosted + 1
        };
        setJobStats(updatedStats);
        localStorage.setItem("jobStats", JSON.stringify(updatedStats));
