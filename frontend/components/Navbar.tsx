...
export default function Navbar() {
  ...
  // ✅ SUBSCRIPTION VISIBILITY LOGIC (COPIED FROM CURRENT NAVBAR)
  const showSubscription = !user || user.role === "EMPLOYER";
    ...
    <header className="fixed top-0 left-0 w-full bg-white z-50 shadow-sm">
      <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-10 text-sm font-medium text-gray-800">
          ...
          {/* ✅ SUBSCRIPTION LINK (NEW) */}
          {showSubscription && (
            <Link href="/subscription" className="hover:text-black">
              Subscription
            </Link>
          )}

          {user?.role === "CANDIDATE" && (
            <Link href="/jobs/all-jobs" className="hover:text-black">
              Apply for Job
            </Link>
          )}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 rounded-full border border-gray-300 px-4 py-2 text-sm hover:bg-gray-50 transition"
                >
                ...
                {isDropdownOpen && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-56 rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 z-50">
                      ...
                      {/* ✅ SUBSCRIPTION IN DROPDOWN (EMPLOYER ONLY) */}
                      {user.role === "EMPLOYER" && (
                        <Link
                          href="/subscription"
                          className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
